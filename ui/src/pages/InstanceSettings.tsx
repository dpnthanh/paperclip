...
export function InstanceSettings() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { label: t("settings.nav.heartbeats", { defaultValue: "Heartbeats" }) },
    ]);
  }, [setBreadcrumbs, t]);
  ...
  const toggleMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("settings.instance.updateHeartbeatFailed", { defaultValue: "Failed to update heartbeat." }));
    },
  });
  ...
  const disableAllMutation = useMutation({
    mutationFn: async (agentRows: InstanceSchedulerHeartbeatAgent[]) => {
      ...
      if (failures.length > 0) {
        ...
        const detail = firstError instanceof Error ? firstError.message : t("settings.instance.unknownError", { defaultValue: "Unknown error" });
        throw new Error(
          failures.length === 1
            ? t("settings.instance.disableOneFailed", { defaultValue: "Failed to disable 1 timer heartbeat: {{detail}}", detail })
            : t("settings.instance.disableManyFailed", { defaultValue: "Failed to disable {{count}} of {{total}} timer heartbeats. First error: {{detail}}", count: failures.length, total: enabled.length, detail }),
        );
      }
      return enabled;
    },
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("settings.instance.disableAllFailed", { defaultValue: "Failed to disable all heartbeats." }));
    },
  });
  ...
  if (heartbeatsQuery.isLoading) {
    return <div className="text-sm text-muted-foreground">{t("settings.instance.loading", { defaultValue: "Loading scheduler heartbeats..." })}</div>;
  }
    ...
    return (
      <div className="text-sm text-destructive">
        {heartbeatsQuery.error instanceof Error
          ...
          : t("settings.instance.loadFailed", { defaultValue: "Failed to load scheduler heartbeats." })}
      </div>
    );
  }
    ...
    <div className="max-w-5xl space-y-6">
      <div className="space-y-2">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">{t("settings.instance.title", { defaultValue: "Scheduler Heartbeats" })}</h1>
        </div>
        <p className="text-sm text-muted-foreground">
          {t("settings.instance.description", { defaultValue: "Agents with a timer heartbeat enabled across all of your companies." })}
        </p>
      </div>

      <div className="flex items-center gap-4 text-sm text-muted-foreground">
        <span><span className="font-semibold text-foreground">{activeCount}</span> {t("settings.instance.active", { defaultValue: "active" })}</span>
        <span><span className="font-semibold text-foreground">{disabledCount}</span> {t("settings.instance.disabled", { defaultValue: "disabled" })}</span>
        <span><span className="font-semibold text-foreground">{grouped.length}</span> {grouped.length === 1 ? t("settings.instance.companyNoun_one", { defaultValue: "company" }) : t("settings.instance.companyNoun_other", { defaultValue: "companies" })}</span>
        {anyEnabled && (
          <Button
            variant="destructive"
            size="sm"
            ...
            onClick={() => {
              if (!window.confirm(t("settings.instance.confirmDisableAll", { defaultValue: "Disable timer heartbeats for all {{count}} enabled agents?", count: enabledCount }))) {
                return;
              }
              disableAllMutation.mutate(agents);
            }}
          >
            {disableAllMutation.isPending ? t("settings.instance.disabling", { defaultValue: "Disabling..." }) : t("settings.instance.disableAll", { defaultValue: "Disable All" })}
          </Button>
        )}
      </div>
      ...
      {agents.length === 0 ? (
        <EmptyState
          icon={Clock3}
          message={t("settings.instance.empty", { defaultValue: "No scheduler heartbeats match the current criteria." })}
        />
      ) : (
                  ...
                  {group.agents.map((agent) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          {agent.schedulerActive ? t("settings.instance.on", { defaultValue: "On" }) : t("settings.instance.off", { defaultValue: "Off" })}
                        </Badge>
                        <Link
                        ...
                        >
                          {agent.lastHeartbeatAt
                            ...
                            : t("settings.instance.never", { defaultValue: "never" })}
                        </span>
                        <span className="ml-auto flex items-center gap-1.5 shrink-0">
                          <Link
                            to={buildAgentHref(agent)}
                            className="text-muted-foreground hover:text-foreground"
                            title={t("settings.instance.fullAgentConfig", { defaultValue: "Full agent config" })}
                          >
                          ...
                          >
                            {saving ? "..." : agent.heartbeatEnabled ? t("settings.instance.disableTimerHeartbeat", { defaultValue: "Disable Timer Heartbeat" }) : t("settings.instance.enableTimerHeartbeat", { defaultValue: "Enable Timer Heartbeat" })}
                          </Button>
                        </span>
                      </div>
                    );
