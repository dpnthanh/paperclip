...
export function ThemeToggle({ className, variant = "icon", onAfterToggle }: ThemeToggleProps) {
  ...
  const label = isDark
    ? t("common.theme.switchToLight", { defaultValue: "Switch to light mode" })
    : t("common.theme.switchToDark", { defaultValue: "Switch to dark mode" });
    ...
    return (
      ...
      >
        ...
        <span className="min-w-0 flex-1">
          ...
          <span className="block text-xs text-muted-foreground">{t("common.theme.description", { defaultValue: "Toggle the app appearance." })}</span>
        </span>
      </button>
    );
  }
