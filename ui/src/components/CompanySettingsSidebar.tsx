    ...
    <aside className="w-full h-full min-h-0 border-r border-border bg-background flex flex-col">
      <div className="flex flex-col gap-1 px-3 py-3 shrink-0">
        ...
        >
          ...
          <span className="truncate">{selectedCompany?.name ?? t("sidebar.companyFallback", { defaultValue: "Company" })}</span>
        </Link>
        <div className="flex items-center gap-2 px-2 py-1">
          <Settings className="h-4 w-4 text-muted-foreground shrink-0" />
          <span className="flex-1 truncate text-sm font-bold text-foreground">
            {t("sidebar.companySettingsTitle", { defaultValue: "Company Settings" })}
          </span>
        </div>
      </div>
      ...
      <nav className="flex-1 min-h-0 overflow-y-auto scrollbar-auto-hide px-3 py-2">
        <div className="px-3 pb-1 text-(length:--text-micro) font-semibold uppercase tracking-wide text-muted-foreground">
          {t("sidebar.companySettingsSection", { defaultValue: "Company settings" })}
        </div>
        <div className="flex flex-col gap-0.5">
          <SidebarNavItem to="/company/settings" label={t("settings.nav.general", { defaultValue: "General" })} icon={SlidersHorizontal} end />
          {showCloudUpstream ? (
            <SidebarNavItem
              to="/company/settings/cloud-upstream"
              label={t("sidebar.nav.cloudUpstream", { defaultValue: "Cloud upstream" })}
              icon={CloudUpload}
              end
            />
          ) : null}
          <SidebarNavItem
            ...
            label={t("sidebar.nav.members", { defaultValue: "Members" })}
            icon={Users}
          ...
          <SidebarNavItem to="/company/settings/invites" label={t("sidebar.nav.invites", { defaultValue: "Invites" })} icon={MailPlus} end />
          <SidebarNavItem to="/company/settings/secrets" label={t("sidebar.nav.secrets", { defaultValue: "Secrets" })} icon={KeyRound} end />
        </div>
        <div className="mt-5 px-3 pb-1 text-(length:--text-micro) font-semibold uppercase tracking-wide text-muted-foreground">
          {t("settings.nav.instanceSettings", { defaultValue: "Instance settings" })}
        </div>
        <div className="flex flex-col gap-0.5">
          <SidebarNavItem
            to={`${INSTANCE_SETTINGS_PATH_PREFIX}/profile`}
            label={t("sidebar.nav.profile", { defaultValue: "Profile" })}
            icon={UserRoundPen}
            end
          />
          <SidebarNavItem
          ...
          <SidebarNavItem
            ...
            label={t("sidebar.nav.environments", { defaultValue: "Environments" })}
            icon={MonitorCog}
            end
          />
          <SidebarNavItem
            to={`${INSTANCE_SETTINGS_PATH_PREFIX}/access`}
            label={t("sidebar.nav.access", { defaultValue: "Access" })}
            icon={Shield}
            end
          />
          <SidebarNavItem
            to={`${INSTANCE_SETTINGS_PATH_PREFIX}/heartbeats`}
            label={t("sidebar.nav.heartbeats", { defaultValue: "Heartbeats" })}
            icon={Clock3}
            end
          />
          <SidebarNavItem
            to={`${INSTANCE_SETTINGS_PATH_PREFIX}/experimental`}
            label={t("sidebar.nav.experimental", { defaultValue: "Experimental" })}
            icon={FlaskConical}
          />
          <SidebarNavItem
            to={`${INSTANCE_SETTINGS_PATH_PREFIX}/plugins`}
            label={t("sidebar.nav.plugins", { defaultValue: "Plugins" })}
            icon={Puzzle}
          />
          ...
          <SidebarNavItem
            ...
            label={t("sidebar.nav.adapters", { defaultValue: "Adapters" })}
            icon={Cpu}
          />
        </div>
      </nav>
